  ...
  return (
    <div className="pizza-block">
      ...
      <div className="pizza-block__selector">
        <ul>
          {props.types.map((type, idx) => (
            <li onClick={() => setActiveType(idx)} className={activeType === idx ? 'active' : ''}>
              {doughType[type]}
            </li>
          ))}
        </ul>
        <ul>
          {props.sizes.map((size, idx) => (
            <li onClick={() => {setActiveSize(idx); setActivePrice(idx);}} className={activeSize === idx ? 'active' : ''}>
              {size} см
            </li>
          ))}
        </ul>
      ...
      <div>
        ...
        <button className="pizza-block__button" onClick={() => onClickDeleteButton()}>-</button>
        <span>В корзине</span>
        <span className="pizza-block__amount">{amount}</span>
        <button className="pizza-block__button" onClick={() => onClickAddButton()}>+</button>
      </div>
    </div>
  );
}
